...
uint16_t display_mode=0;                                  // 0:FFT/waterfall, 1:IQ, 2:Audio, 3: TXT
...
BUTTON bandMenu[MAXBAND] = {
  {"1.8", BANDMENUX,     BANDMENUX+95,  BANDMENUY+135, BANDMENUY+175, DARKGREY, GREY, WHITE},
  {"3.5", BANDMENUX+100, BANDMENUX+195, BANDMENUY+135, BANDMENUY+175, DARKGREY, GREY, WHITE},
  {"7",   BANDMENUX+200, BANDMENUX+295, BANDMENUY+135, BANDMENUY+175, DARKGREY, GREY, WHITE},
  {"10",  BANDMENUX,     BANDMENUX+95,  BANDMENUY+90, BANDMENUY+130, DARKGREY, GREY, WHITE},
  {"14",  BANDMENUX+100, BANDMENUX+195, BANDMENUY+90, BANDMENUY+130, DARKGREY, GREY, WHITE},
  {"18",  BANDMENUX+200, BANDMENUX+295, BANDMENUY+90, BANDMENUY+130, DARKGREY, GREY, WHITE},
  {"21",  BANDMENUX,     BANDMENUX+95,  BANDMENUY+45, BANDMENUY+85, DARKGREY, GREY, WHITE},
  {"24",  BANDMENUX+100, BANDMENUX+195, BANDMENUY+45, BANDMENUY+85, DARKGREY, GREY, WHITE},
  {"28",  BANDMENUX+200, BANDMENUX+295, BANDMENUY+45, BANDMENUY+85, DARKGREY, GREY, WHITE},
  {"144", BANDMENUX,     BANDMENUX+95,  BANDMENUY,  BANDMENUY+40, DARKGREY, GREY, WHITE},
...
BUTTON funcMenu[MAXFUNC] = {
  {"P/ATT", FUNCMENUX,    FUNCMENUX+90, FUNCMENUY+130,FUNCMENUY+220,DARKGREY,GREY,WHITE},
  {"  S  ", FUNCMENUX+95, FUNCMENUX+185,FUNCMENUY+130,FUNCMENUY+220,DARKGREY,GREY,WHITE},
  {"Displ", FUNCMENUX+190,FUNCMENUX+280,FUNCMENUY+130,FUNCMENUY+220,DARKGREY,GREY,WHITE},
  {" AGC ", FUNCMENUX+285,FUNCMENUX+375,FUNCMENUY+130,FUNCMENUY+220,DARKGREY,GREY,WHITE},
  {"Shift", FUNCMENUX,    FUNCMENUX+90, FUNCMENUY+30,FUNCMENUY+120, DARKGREY,GREY,WHITE},
  {"Baud",  FUNCMENUX+95, FUNCMENUX+185,FUNCMENUY+30,FUNCMENUY+120, DARKGREY,GREY,WHITE},
  {"Back ", FUNCMENUX+370,FUNCMENUX+465,FUNCMENUY+10,FUNCMENUY+45,  DARKGREY, GREY, WHITE},
};
...
int bandFreq[MAXBAND][3] = {
  ...
  {1000000, 100000000, 0},
  {28000000, 32000000, 1268},                // 1.3 GHz
};
...
void ui_handler()
{
  ...
  displayTuningRTTY();
  if (display_mode == TXT) {
    if (mode == MODE_RTTY) displayDecodedText();
    else if (mode == MODE_CW) displayDecodedCW();
  }
  displaySmeter();
  displayTime();
/*
...
*/
  switch (state) {
    ...
    if (event == EV_TOUCHDOWN) { 
      ...
      if (y>FFTY-30 && y<FFTY+52) {               // new frequency?
        int df = map(x, 0, 480, -span/2, span/2);
        if (abs(df) <= span/2) {
   	      freq += df*1000;
        }
      }
      else {
        subState = SS_TOUCHDOWN;                  // handle main menu in loop
      }
    }
...
//          fft_gain = logtbl[40+fft_lvl];
        ...
        case MODE:
          ...
          break;
        case REF:
          if (display_mode != TXT) state = S_REFLVL;
          break;
        case FUNC:
          state = S_FUNC;
          hold = true;
   ...
   /*
    ...
    if (subState == SS_INIT) {
      xHoldL = 250;
      xHoldH = 390;
      fillRect(250, 38, 390, 98, LIGHTBLUE);
    ...
    if(subState == SS_SHOW) {
      for (int i=0; i<MAXFUNC; i++) {
        ...
        *s = 0;
        switch (i) {
          case 0: sprintf(s, "%s",  "OFF"); break;
          case 1: sprintf(s, "%s", (smode==0) ? "Bar" : "dB "); break;
          case 2: sprintf(s, "%s", (display_mode==FFT) ? " FFT" : (display_mode==IQ) ? " IQ " : (display_mode==AU) ? "Audio" : "Decod"); break;
    ...
    if (subState == SS_TOUCHDOWN) {
      buttonPress(funcMenu, MAXFUNC);                         // read button pressed
      if (event == EV_TOUCHUP) {
        funcMenu[button].press = false;
        if (button>=0) {
          setFont(1);
          ...
          case 1:
            ...
            break;
          case 2:
//            int lastDisplay_mode = display_mode;
            if (++display_mode == 4) display_mode = 0;
//            if (lastDisplay_mode == TXT) {
//
//            }
            subState = SS_SHOW;
            break;
    ...
    if (subState == SS_INIT) {
      ...
      drawRoundRect(BANDMENUX-5, BANDMENUY, BANDMENUX+300, BANDMENUY+180, DARKGREY);
      for (int i=0; i<MAXBAND; i++) {
    ...
    if (subState == SS_TOUCHDOWN) {
      ...
      if (event == EV_TOUCHUP) {
        ...
        fillRect(BANDMENUX-5, BANDMENUY, BANDMENUX+300, BANDMENUY+225, BLACK);
        displayMain();
//        displaySpan();
        displayMode();
        displayFreq();
...
void setBand(band) {
  uint8_t sw;
  switch (band) {
    case 0: sw=0x01; break;     // 1.8
    case 1: sw=0x02; break;     // 3.5
    case 2:                     // 7
    case 3: sw=0x04; break;     // 10
    case 4:                     // 14
    case 5:                     // 18
    case 6: sw=0x08; break;     // 21
    case 7:                     // 24
    case 8: sw=0x10; break;     // 28
    case 9: sw=0x10; break;     // 144
  }
...
void displayFreq()
{
  ...
  sprintf(freqString, "%llu", f);                               // i.e. 340091875

  freqLen = strlen(freqString);
  if (freqLen != lastFreqLen) {                                 // if # digits changed
    for (int i=0; i<0x30; i++) {                                // fontheight=0x30
      drawHLine(80, FREQPOSY+i, 400, BLACK);                    // remove any left over junk
    }
  }
  x = 240-32*freqLen/2;                                         // start of char after freqString
